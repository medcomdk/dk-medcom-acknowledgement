...
Changes to the IG are automatically built and viewed at http://medcomfhir.dk/ig/acknowledgement as an HL7 Implementation Guide. Profiles can also be viewed on Simplifier at https://simplifier.net/medcom-fhir-messaging

The IG can be loaded up in a FHIR server instance using the following docker command:

```
docker run --name=hapi-fhir --rm -p 8080:8080 -e HAPI_IMPLEMENTATION_GUIDE_URL=http://medcomfhir.dk/ig/acknowledgement/package.tgz mzgtrifork/hapi-fhir-jpaserver-starter:0.0.1
```
